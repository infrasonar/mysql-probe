...
def get_stats_from_innodb_status(innodb_status_text):
    ...
    for line in innodb_status_text.splitlines():
        ...
        # FILE I/O
            ...
            try:
                if len(row) == 8:
                    ...
                    if _are_values_numeric(row[4:8]) and \
                            _are_values_numeric(row[11:15]):
                        results['Innodb_pending_normal_aio_reads'] = sum(
                            map(int, (row[4], row[5], row[6], row[7]))
                        )
                        results['Innodb_pending_normal_aio_writes'] = sum(
                            map(int, (row[11], row[12], row[13], row[14]))
                        )

                    # (len(row) == 16) Pending normal aio reads: 0
                    # [0, 0, 0, 0] , aio writes: 0 [0, 0] ,
                    elif _are_values_numeric(row[4:9]) and \
                            _are_values_numeric(row[12:15]):
        ...
        # INSERT BUFFER AND ADAPTIVE HASH INDEX
            ...
            results['Innodb_ibuf_merged'] = sum(
    ...
    try:
        results['Innodb_checkpoint_age'] = results['Innodb_lsn_current'] - \
            results['Innodb_lsn_last_checkpoint']
    ...
    try:
        ...
        if 'Innodb_buffer_pool_bytes_used' not in results:
            results['Innodb_buffer_pool_bytes_used'] = \
    ...
    #     logging.debug("Collecting Extra Innodb Metrics")
